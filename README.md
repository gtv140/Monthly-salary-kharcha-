...
<title>Pocket Tracker - WebHub Pro</title>
...
function drawChart(){
  ...
  const remainingArr=dailyData.map((d,i)=>{
    let total=dailyData.slice(0,i+1).reduce((a,b)=>a+b.total,0);
    return Math.max(0,salary+initialSaving-total-loanAmount);
  });
  ...
  window.expenseChartObj=new Chart(ctx,{
    ...
    data:{
      labels:labels,
      datasets:[
        {label:'Daily Expense',data:expenses,borderColor:'#FF5733',backgroundColor:'rgba(255,87,51,0.2)',fill:true,tension:0.3},
        {label:'Remaining Balance',data:remainingArr,borderColor:'#33C3FF',backgroundColor:'rgba(51,195,255,0.2)',fill:true,tension:0.3}
      ]
    },
